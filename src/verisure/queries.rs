...
pub fn door_lock_query(giid: &str) -> Value {
    json!({
        ...
        "variables": { "giid": giid },
        "query": "query DoorLock($giid: String!) { installation(giid: $giid) { smartLocks { device { deviceLabel area } lockStatus doorState lockMethod eventTime doorLockType secureMode userString } } }"
    })
}
